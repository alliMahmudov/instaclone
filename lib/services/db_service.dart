...
class DBService {
  ...
  static Future<Member> loadUser(Member someone) async {

    var value = await _firestore.collection(folder_users).doc(someone.uid).get();
    Member member = Member.fromJson(value.data()!);
  ...
  static Future removePostsFromMyFeeds(Member someone) async {
    List<Post> posts = [];
    var querySnapshot = await _firestore
        .collection(folder_users)
        .doc(someone.uid)
